...
- sov 情感分析否定前缀改词级甄别,修掉「非常推荐」「特别好用」「无论如何…值得」误判与双重否定误判(新增回归测试)。
- 评分 D4 篇幅分档补上 (2500,3000] 区间,不再把偏长页面误报成「过短」。
- A4 站点级封锁判定复用 _ua_allowed,Allow: / 覆盖 Disallow: / 时不再与 A1-A3 自相矛盾。
- schema/hreflang 对缺分隔符的 --qa/--step/--locale 打印格式说明并返回 2,不再抛 traceback。
- batch_score 排序错误页沉底,不再排在 0 分弱页之前误导「弱页优先」。
- run_cases.py 输入文件改用 with 读取,补上唯一一处文件句柄泄漏。
- D2 检查名改为「前 140 字符」,检查名与方法论文档对齐实际实现口径。
- 用户提供的 JSON 文件(records/aliases/facts/psi)解析失败统一打印原因并返回 2,不再抛 traceback 暴露本地路径。
- 百度推送脚本注释提示官方 http 明文接口的 token 泄露风险。
- SARIF driver 版本号与 README tests badge 对齐当前版本与实测数。
...
- 只收实体不存在的明确标记,不收裸「不存在 / 没有」,避免误杀「黑哥AI没有这个功能」这类真提及。
- 修复后真实采集复测:黑哥AI 在秘塔的问句覆盖率从误报 33% 校正为真实 0%,竞品即梦 100%。

### 真实采集闭环跑通

- 首次端到端跑通「浏览器采集 → records.json → measure」:秘塔 AI 搜索游客可用、无需登录,作为可达的国产 AI 引擎样本(豆包/DeepSeek 需登录态,仍待宿主浏览器)。
- 产出真实监测报告(黑哥AI 在秘塔零可见度,印证作战手册「瓶颈在内容质量层、先补内容再投放」的判断)。
